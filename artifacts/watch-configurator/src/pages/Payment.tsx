...
export default function Payment() {
  ...
  const { data: order, isLoading, refetch } = useGetOrder(Number(orderId), {
      ...
      <div className="liquid-glass rounded-3xl p-8 max-w-sm w-full flex flex-col items-center text-center relative z-10 gap-5">
        ...
        {isPaid ? (
          ...
          <>
            ...
            <a href={deepLink} className="w-full" onClick={e => {
              // On desktop tg:// may not work — open web link as fallback
            ...
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              <div className={cn(
                ...
                order.status === 'pending' ? 'bg-amber-400 animate-pulse' : 'bg-green-400'
              )} />
              {order.status === 'pending' ? 'Ожидание оплаты…' : `Статус: ${order.status}`}
            </div>
          </>
        )}
      </div>
